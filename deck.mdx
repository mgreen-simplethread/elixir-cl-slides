...
<Snippet
  ...
  steps={[
    ...
    { lines: [2], notes: 'Attributes are resolved to their values at compile time, like constants' },
    { range: [4, 10], notes: 'Modules can have multiple function clauses with the same name. Pattern matching and guards are used to invoke the correct one.' },
    { range: [8, 10], notes: 'Guard clauses check arguments, and must use pure functions with no side effects' }
  ]}
/>
...
<Snippet
  ...
  steps={[
    { notes: 'Elixir has a few interesting conditionals besides "if" and "unless"' },
    { range: [7, 12], notes: 'The "case" statement does pattern matching on a single value' },
    { lines: [8], notes: 'Can partially match the value based on the pattern specified' },
    { lines: [9], notes: 'Can also bind variables that can be used in the body of the conditionally executed expression' },
    { lines: [10], notes: 'Underscore is a wildcard - matches anything and everything' }
  ]}
/>
